...
create table td_todo (
    ...
    todo_tmp_cont varchar(2000),
    todo_desc varchar(2000),
    complet_condi_val varchar(128),
    todo_typ_cd enum ('WAKEUP','SLEEP','CLEAN','DIARY','EXERCISE','READING','EXAM','ETIQUET','LEARNING','PRESENT') not null,
    aplyto_ord integer, 
    
    todo_cre_cd enum ('DAILY','WEEK','MONTH','DIRECT'),
    todo_cre_dtl_val varchar(128),
    date_limit_cnt integer,
    todo_point integer not null,
    confm_meth_cd enum ('SCORE','ENG_CNT','WORD_CNT','TIME','MERGE_TIME','CHECKER'),
